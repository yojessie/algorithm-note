...
const [n, a, b, c] = require("fs").readFileSync("/dev/stdin").toString().trim().split("\n");
const card = a.split(" ").map(Number).sort((a, b) => a - b)
...
function countByRange(arr, left, right) {
    let leftIdx = lowerBound(arr, left, 0, arr.length)
    let rightIdx = upperBound(arr, right, 0, arr.length)
    return rightIdx - leftIdx
}

let answer = ''
for (let i of check) {
    let result = countByRange(card, i, i)
    answer += `${result} `
}
console.log(answer)
